...
public class UserDetailServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		return new User(username, userAccount.getPassword(), true, true, passwordNonExpired(userAccount), true,
				AuthorityUtils.createAuthorityList(roles));
	}

	private boolean passwordNonExpired(UserAccount userAccount) {
		return !userAccount.getPwExpirationDate().isBefore(userAccount.getPwExpirationDate().plusYears(1));
	}
